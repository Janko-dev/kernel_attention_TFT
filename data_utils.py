...
def process_favorita(config):
    ...
    if not os.path.exists(os.path.join(config.data_folder, 'tmp.csv')):
        ...
        temporal['traj_id'] = temporal['store_nbr'].astype(str) + '_' + temporal['item_nbr'].astype(str)
        temporal['unique_id'] = temporal['traj_id'] + '_' + temporal['date'].astype(str)
        ...
        resampled_dfs = []
        for traj_id, raw_sub_df in tqdm(temporal.groupby('traj_id')):
            ...
            sub_df[['store_nbr', 'item_nbr', 'onpromotion']] = sub_df[['store_nbr', 'item_nbr', 'onpromotion']].infer_objects(copy=False).ffill()
        ...
        temporal.to_csv(os.path.join(data_folder, 'tmp.csv'))

    else:
        print("resume processing")
        temporal = pd.read_csv(os.path.join(data_folder, 'tmp.csv'), index_col=0)
    ...
    tmp_dates = pd.DataFrame(np.zeros(oil.shape), index=oil.index)
    for d in dates:
        if d in tmp_dates.index:
            tmp_dates.loc[d] = 1
    ...
    temporal = temporal.join(
        oil.loc[tmp_dates.values == 1].ffill(), on='date', how='left')
    temporal['oil'] = temporal['oil'].fillna(-1)

    # oil.name = 'oil'
    # oil.index = pd.to_datetime(oil.index)
    # temporal = temporal.join(
    #     oil.loc[dates].ffill(), on='date', how='left')
    # temporal['oil'] = temporal['oil'].fillna(-1)
